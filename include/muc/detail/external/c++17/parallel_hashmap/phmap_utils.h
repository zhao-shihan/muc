...
struct Hash {
    template<class U,
             typename std::enable_if<has_hash_value<U>::value, int>::type = 0>
...
struct Hash<unsigned char> :
    public phmap_unary_function<unsigned char, size_t> {
...
H HashStateBase<H>::combine(H seed, const T& v, const Ts&... vs) {
    return HashStateBase<H>::combine(
        Combiner<H, sizeof(H)>()(seed, phmap::Hash<T>()(v)), vs...);
}
...
private:
    ...
    template<size_t I = 0, class TUP>
        ...
        I<std::tuple_size<TUP>::value, size_t>::type
        _hash_helper(size_t seed, const TUP& t) const noexcept {
        const auto& el = std::get<I>(t);
        using el_type = typename std::remove_cv<
            typename std::remove_reference<decltype(el)>::type>::type;
        seed = Combiner<size_t, sizeof(size_t)>()(seed,
                                                  phmap::Hash<el_type>()(el));
