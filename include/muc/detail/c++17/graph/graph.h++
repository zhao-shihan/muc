...
#include "muc/detail/c++17/ptr_vector/unique_ptr_vector.h++"
...
public:
    ...
    public:
        ...
        auto
        Adjacent(const Vertex& vertex) const -> std::optional<EdgeIterator> {
            const auto it{fNeighbor.find(vertex)};
            if (it == fNeighbor.cend()) {
                return {};
            }
            return it->second;
        }
    ...
    private:
        T fValue;
        std::pmr::unordered_map<VertexIterator, const EdgeIterator,
                                NeighborHash>
            fNeighbor;
    ...
    public:
        ...
        auto Vertex1() const -> const Vertex& {
            return fVertex1;
        }
        auto Vertex2() const -> const Vertex& {
            return fVertex2;
        }
        auto Weight() const -> const W& {
            return fWeight;
        }
...
public:
    Graph() :
        fMemoryPool{},
        fVertex{VertexListType{&fMemoryPool}, VertexIteratorList{&fMemoryPool}},
        fEdge{EdgeListType{&fMemoryPool}, EdgeIteratorList{&fMemoryPool}} {}
    ...
    auto AddEdge(Vertex& vertex1, Vertex& vertex2,
                 const W& weight) -> EdgeIterator {
    ...
    /// @param weight edge weight
    /// @return An iterator to the edge added.
    auto AddEdge(VertexIterator vertex1, VertexIterator vertex2,
                 const W& weight) -> EdgeIterator {
    ...
    auto VertexList() const -> const auto& {
        return fVertex.list;
    }
    auto EdgeList() const -> const auto& {
        return fEdge.list;
    }
...
protected:
    ...
    auto AddEdgeImpl(Vertex& vertex1, Vertex& vertex2,
                     WW&& weight) -> EdgeIterator {
