...
struct is_trivially_copy_constructible :
    std::integral_constant<bool, __has_trivial_copy(
...
struct is_trivially_copy_assignable :
    std::integral_constant<bool, __has_trivial_assign(
                                     typename std::remove_reference<T>::type) &&
...
private:
    ...
    struct ConstantIteratorsImpl<
        P, phmap::void_t<typename P::constant_iterators>> :
...
public:
    ...
    // `value_type`. It's allowed for `k` to alias `xs...`, and for both to
    // alias `ts...`. The key won't be touched once `xs...` are used to
    // construct an element; `ts...` won't be touched at all, which allows
    // `apply()` to consume any rvalues among them.
    //
    ...
    // constructible from `Ts&&...`, either SFINAE or a hard compile error is
    // OK.
    //
    ...
    // Returns the "value" (as opposed to the "key") portion of the element.
    // Used by maps to implement `operator[]`, `at()` and `insert_or_assign()`.
...
#define PHMAP_INTERNAL_INLINE_CONSTEXPR(var_type, name, init)                \
    ...
    constexpr ::phmap::internal::identity_t<var_type>                        \
        PhmapInternalInlineVariableHolder##name<                             \
            PhmapInternalDummy>::kInstance;                                  \
                                                                             \
...
struct StrippedAccept {
    ...
    struct Accept :
        Derived::template AcceptImpl<typename std::remove_cv<
...
struct Callable {
    // Callable doesn't have Accept because it's the last clause that gets
    // picked when none of the previous clauses are applicable.
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 4365) // '=': conversion from 'T' to 'T',
...
struct allocator_traits {
    ...
    using propagate_on_container_swap = memory_internal::ExtractOrT<
        memory_internal::GetPropagateOnContainerSwap, Alloc, std::false_type>;
    ...
    // rebind_alloc:
    // Alloc::rebind<T>::other if present, otherwise Alloc<T, Args> if this
    // Alloc is Alloc<U, Args>
    template<typename T>
...
protected:
    ...
    void construct(Args&&... args) {
        ...
        ::new (static_cast<void*>(&this->dummy_))
...
template<typename T>
class optional :
    private optional_internal::optional_data<T>,
...
public:
    ...
    // Constructs an `optional` holding an empty value, NOT a default
    // constructed `T`.
    ...
    // `std::forward<Args>(args)...`. (The `in_place_t` is a tag used to
    // indicate that the contained object should be constructed in-place.)
...
// constructors.
//
// A `Span<T>` is somewhat analogous to an `phmap::string_view`, but for an
// array of elements of type `T`. A user of `Span` must ensure that the data
// being pointed to outlives the `Span` itself.
//
...
// accept a variety of array types (e.g. a `std::vector`,
// `phmap::InlinedVector`, a C-style array, etc.). Instead of creating overloads
// for each case, you can simply specify a `Span` as the argument to such a
// routine.
//
// Example:
//
...
public:
    ...
    // this constructor only when the `std::initializer_list` itself outlives
    // the span. In order to meet this requirement it's sufficient to ensure
    // that neither the span nor a copy of it is used outside of the expression
    ...
    //
    //   // Okay: the std::initializer_list<int> will reference a temporary
    //   array
    ...
    Span(std::initializer_list<value_type>
             v) noexcept // NOLINT(runtime/explicit)
        :
        Span(v.begin(), v.size()) {}
    ...
    // Span::empty()
    //
    // Returns a boolean indicating whether or not this span is considered
    // empty.
    ...
    // Span::subspan()
    //
    // Returns a `Span` starting at element `pos` and of length `len`. Both
    // `pos` and `len` are of type `size_type` and thus non-negative. Parameter
    // `pos` must be <= size(). Any `len` value that points past the end of the
    // span will be trimmed to at most size() - `pos`. A default `len` value of
    // `npos` ensures the returned subspan continues until the end of the span.
    //
    // Examples:
    //
    ...
    friend H AbslHashValue(H h, Span v) {
        return H::combine(
            H::combine_contiguous(std::move(h), v.data(), v.size()), v.size());
    }
...
public:
    ...
    // Alignment of the layout, equal to the strictest alignment of all
    // elements. All pointers passed to the methods of layout must be aligned to
    // this value.
...
void* Allocate(Alloc* alloc, size_t n) {
    ...
    A mem_alloc(*alloc);
    void* p = &*AT::allocate(mem_alloc,
                             (n + sizeof(M) - 1) /
                                 sizeof(M)); // `&*` to support custom pointers
                                             // such as boost offset_ptr.
    assert(reinterpret_cast<uintptr_t>(p) % Alignment == 0 &&
...
public:
    ...
    static constexpr bool value =
        std::is_standard_layout<K>() && std::is_standard_layout<Pair>() &&
...
private:
    static void emplace(slot_type* slot) {
        // The construction of union doesn't do anything at runtime but it
        // allows us to access its members without violating aliasing rules.
        new (slot) slot_type;
    }
...
public:
    ...
    public:
        ...
        bool owns_shared_lock() const noexcept {
            return locked_shared_;
        }
...
//    Lockable m;
//
//    Lockable::ReadWriteLock read_lock(m); // take a lock (read if supported,
//    otherwise write)
//    ... do something
//
//    m.switch_to_unique(); // returns true if we had a read lock and switched
//    to write
