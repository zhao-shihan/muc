...
#pragma warning(disable : 4365) // conversion from 'int' to 'const unsigned
...
public:
    static constexpr bool kValue =
        (phmap::is_trivially_copyable<ExtentsRemoved>::value ||
         !kIsCopyOrMoveConstructible) &&
        (phmap::is_trivially_copy_assignable<ExtentsRemoved>::value ||
...
// This declaration prevents global `swap` and `phmap::swap` overloads from
// being considered unless ADL picks them up.
void swap();
...
public:
    ...
    operator==(weak_equality v, compare_internal::OnlyLiteralZero<>) noexcept {
        return v.value_ == 0;
    }
    friend constexpr bool
    operator!=(weak_equality v, compare_internal::OnlyLiteralZero<>) noexcept {
...
public:
    ...
    // Comparisons
    friend constexpr bool
    operator==(weak_ordering v, compare_internal::OnlyLiteralZero<>) noexcept {
        return v.value_ == 0;
    }
    friend constexpr bool
    operator!=(weak_ordering v, compare_internal::OnlyLiteralZero<>) noexcept {
...
public:
    ...
    friend constexpr bool
    operator<(strong_ordering v, compare_internal::OnlyLiteralZero<>) noexcept {
        return v.value_ < 0;
    }
    friend constexpr bool
    operator<=(strong_ordering v,
...
constexpr phmap::weak_ordering
compare_result_as_ordering(const phmap::weak_ordering c) {
    return c;
}

template<typename Compare, typename K, typename LK,
         phmap::enable_if_t<
             !std::is_same<bool, phmap::invoke_result_t<Compare, const K&,
                                                        const LK&>>::value,
             int> = 0>
constexpr phmap::weak_ordering
do_three_way_comparison(const Compare& compare, const K& x, const LK& y) {
...
struct common_params {
    // If Compare is a common comparator for a std::string-like type, then we
    // adapt it to use heterogeneous lookup and to be a key-compare-to
    // comparator.
    ...
    enum {
        ...
        // Upper bound for the available space for values. This is largest for
        // leaf nodes, which have overhead of at least a pointer + 4 bytes (for
        // storing 3 field_types and an enum).
        kNodeSlotSpace =
    ...
    // The following methods are necessary for passing this struct as
    // PolicyTraits for node_handle and/or are used within btree.
...
struct map_params :
    common_params<Key, Compare, Alloc, TargetNodeSize, Multi,
                  phmap::priv::map_slot_policy<Key, Data>> {
    using super_type = typename map_params::common_params;
    using mapped_type = Data;
    // This type allows us to move keys when it is safe to do so. It is safe
    // for maps in which value_type and mutable_value_type are layout
    // compatible.
...
struct upper_bound_adapter {
    ...
    bool operator()(const K& a, const LK& b) const {
        ...
        return !phmap::compare_internal::compare_result_as_less_than(
            comp(b, a));
    }
...
private:
    using layout_type =
        phmap::priv::Layout<btree_node*, field_type, slot_type, btree_node*>;
    ...
    // A lower bound for the overhead of fields other than values in a leaf
    // node.
    ...
    // padding.
    constexpr static size_type NodeTargetValues(const int begin,
                                                const int end) {
    ...
    enum {
        ...
        // The node is internal (i.e. is not a leaf node) if and only if
        // `max_count` has this value.
        kInternalNodeMaxCount = 0,
    };

    // Leaves can have less than kNodeValues values.
    constexpr static layout_type
    LeafLayout(const int max_values = kNodeValues) {
    ...
    template<size_type N>
    inline const typename layout_type::template ElementType<N>*
    GetField() const {
        assert(N < 3 || !leaf());
        return InternalLayout().template Pointer<N>(
...
#pragma GCC diagnostic pop
    ...
    template<typename K>
    SearchResult<int, is_key_compare_to::value>
    lower_bound(const K& k, const key_compare& comp) const {
    ...
    template<typename K, typename Compare>
    SearchResult<int, false>
    linear_search_impl(const K& k, int s, const int e, const Compare& comp,
    ...
    template<typename K, typename Compare>
    SearchResult<int, false>
    binary_search_impl(const K& k, int s, int e, const Compare& comp,
                       std::false_type /* IsCompareTo */) const {
        while (s != e) {
            const int mid = (s + e) >> 1;
            if (comp(key(mid), k)) {
    ...
    SearchResult<int, true>
    binary_search_impl(const K& k, int s, int e, const CompareTo& comp,
                       std::true_type /* IsCompareTo */) const {
    ...
    // after that range to the left by to_erase. Does not change children at
    // all.
    ...
    static btree_node* init_leaf(btree_node* n, btree_node* parent,
        ...
        phmap::priv::SanitizerPoisonMemoryRegion(n->slot(0),
                                                 max_cnt * sizeof(slot_type));
        return n;
    }
...
private:
    ...
    // Move n values starting at value i in this node into the values starting
    // at value j in node x.
    void uninitialized_move_n(const size_type n, const size_type i,
                              const size_type j, btree_node* x,
                              allocator_type* alloc) {
        phmap::priv::SanitizerUnpoisonMemoryRegion(x->slot(j),
                                                   n * sizeof(slot_type));
...
class btree {
    ...
    // We use a static empty node for the root/leftmost/rightmost of empty
    // btrees in order to avoid branching in begin()/end().
    struct alignas(node_type::Alignment()) EmptyNodeType : node_type {
        ...
        field_type start = 0;
        field_type count = 0;
        // max_count must be != kInternalNodeMaxCount (so that this node is
        // regarded as a leaf node). max_count() is never called when the tree
        // is empty.
...
private:
    ...
    // x into this btree in their order in x. This btree must be empty before
    // this method is called. This method is used in copy construction, copy
...
public:
    ...
    ~btree() {
        // Put static_asserts in destructor to avoid triggering them before the
        // type is complete.
    ...
    // Requirement: if `key` already exists in the btree, does not consume
    // `args`. Requirement: `key` is never referenced after consuming `args`.
    template<typename... Args>
    std::pair<iterator, bool> insert_unique(const key_type& key,
                                            Args&&... args);

    // Inserts with hint. Checks to see if the value should be placed
    // immediately before `position` in the tree. If so, then the insertion will
    ...
    insert_hint_unique(iterator position, const key_type& key, Args&&... args);
    ...
    // for storing elements divided by the number of elements. Returns 0 for
    // empty trees.
...
private:
    ...
    node_type* allocate(const size_type sz) {
        return reinterpret_cast<node_type*>(
            phmap::priv::Allocate<node_type::Alignment()>(mutable_allocator(),
                                                          (size_t)sz));
    }
    ...
    void deallocate(const size_type sz, node_type* node) {
        phmap::priv::Deallocate<node_type::Alignment()>(mutable_allocator(),
                                                        node, (size_t)sz);
    }
    ...
    // version uses a less-than comparator and is incapable of distinguishing
    // when there is an exact match. The second version is for the
    // key-compare-to specialization and distinguishes exact matches. The
    // key-compare-to specialization allows the caller to avoid a subsequent
    // comparison to determine if an exact match was made, which is important
    // for keys with expensive comparison, such as strings.
    template<typename K>
    SearchResult<iterator, is_key_compare_to::value>
    internal_locate(const K& key) const;
...
template<typename P>
inline void
btree_node<P>::remove_values_ignore_children(int i, size_type to_erase,
                                             allocator_type* alloc) {
...
void btree_node<P>::rebalance_left_to_right(const int to_move,
    ...
    // other (to_move - 1) values in the left node are moved into the right
    // node. Lastly, a new delimiting value is moved from the left node into the
    // parent, and the remaining empty left node entries are destroyed.

    if (right->count() >= to_move) {
        // The original location of the right->count() values are sufficient to
        // hold the new to_move entries from the parent and left node.

        // 1) Shift existing values in the right node to their correct
        // positions.
        ...
        // 3) Move the (to_move - 1) values from the left node to the right
        // node.
    ...
    params_type::move(alloc, slot(count() - to_move),
                      parent()->slot(position()));
...
void btree<P>::copy_or_move_values_in_order(Btree* x) {
    ...
    ++iter;
    for (; iter != x->end(); ++iter) {
        // If the btree is not empty, we can just insert the new value at the
        // end of the tree.
...
constexpr bool btree<P>::static_assert_validation() {
    ...
    static_assert(
        std::is_same<compare_result_type, bool>::value ||
            std::is_convertible<compare_result_type,
                                phmap::weak_ordering>::value,
...
inline auto btree<P>::insert_hint_unique(iterator position, const key_type& key,
    ...
    if (!empty()) {
        if (position == end() || compare_keys(key, position.key())) {
            ...
            ++position;
            if (position == end() || compare_keys(key, position.key())) {
                // {original `position`}.key() < key < {current
                // `position`}.key()
                return {internal_emplace(position, std::forward<Args>(args)...),
                        true};
            }
...
auto btree<P>::operator=(const btree& x) -> btree& {
    if (this != &x) {
        ...
        *mutable_key_comp() = x.key_comp();
        if (phmap::allocator_traits<allocator_type>::
    ...
    if (this != &x) {
        ...
        if (phmap::allocator_traits<allocator_type>::
            ...
            if (allocator() == x.allocator()) {
                ...
                // We aren't allowed to propagate the allocator and the
                // allocator is different so we can't take over its memory. We
                // must move each element individually. We need both `x` and
                // `this` to have `x`s key comparator while moving the values so
                // we can't swap the key comparators.
                *mutable_key_comp() = x.key_comp();
                copy_or_move_values_in_order(&x);
            }
        }
...
auto btree<P>::erase(iterator iter) -> iterator {
    ...
    if (!iter.node->leaf()) {
        // Deletion of a value on an internal node. First, move the largest
        // value from our left child here, then delete that position (in
        // remove_value() below). We can get to the largest value from our left
        // child by decrementing iter.
...
auto btree<P>::rebalance_after_delete(iterator iter) -> iterator {
    ...
    for (;;) {
        ...
        // On the first iteration, we should update `res` with `iter` because
        // `res` may have been invalidated.
        if (first_iteration) {
            res = iter;
    ...
    while (size_ > target_size) {
        if (_begin.node->leaf()) {
            ...
            const size_type remaining_in_node =
                _begin.node->count() - _begin.position;
...
void btree<P>::erase_same_node(iterator _begin, iterator _end) {
    ...
    if (!node->leaf()) {
        ...
        for (size_type i = _begin.position + to_erase + 1; i <= node->count();
             ++i) {
    ...
    //   exists
    // * or _end != this->end(), and therefore rightmost_ hasn't been erased,
    // since
    ...
    if (node != root()) {
        if (node->position() > 0) {
            ...
            if (left->count() < kNodeValues) {
                ...
                if (((insert_position - to_move) >= 0) ||
                    ...
                    left->rebalance_right_to_left(to_move, node,
                                                  mutable_allocator());
        ...
        if (node->position() < parent->count()) {
            ...
            if (right->count() < kNodeValues) {
                // We bias rebalancing based on the position being inserted. If
                // we're inserting at the _beginning of the left node then we
                ...
                if ((insert_position <= (node->count() - to_move)) ||
                    ...
                    node->rebalance_left_to_right(to_move, right,
                                                  mutable_allocator());
        ...
        // Create a new root node and set the current root node as the child of
        // the new root.
...
bool btree<P>::try_merge_or_rebalance(iterator* iter) {
    ...
    if (iter->node->position() < parent->count()) {
        ...
        if ((right->count() > kMinNodeValues) &&
            ...
            iter->node->rebalance_right_to_left(to_move, right,
                                                mutable_allocator());
            return false;
        }
    }
    if (iter->node->position() > 0) {
        ...
        if ((left->count() > kMinNodeValues) &&
            ...
            left->rebalance_left_to_right(to_move, iter->node,
                                          mutable_allocator());
...
inline auto btree<P>::internal_emplace(iterator iter, Args&&... args)
    ...
    if (iter.node->count() == max_count) {
        ...
        if (max_count < kNodeValues) {
            // Insertion into the root where the root is smaller than the full
...
btree<P>::internal_locate_impl(const K& key,
    ...
    for (;;) {
        ...
        // equal, but determining equality would require doing an extra
        // comparison on each node on the way down, and we will need to go all
        // the way to the leaf node in the expected case.
        if (iter.node->leaf()) {
            break;
        }
...
btree<P>::internal_verify(const node_type* node, const key_type* lo,
                          const key_type* hi) const {
    assert(node->count() > 0);
    assert(node->count() <= node->max_count());
    if (lo) {
...
public:
    ...
    node_type extract(iterator position) {
        ...
        auto node =
            CommonAccess::Move<node_type>(get_allocator(), position.slot());
        erase(position);
        return node;
    }
...
public:
    ...
    insert_return_type insert(node_type&& node) {
        ...
        std::pair<iterator, bool> res = this->tree_.insert_unique(
...
public:
    ...
    std::pair<iterator, bool> try_emplace(key_type&& k, Args&&... args) {
        // Note: `key_ref` exists to avoid a ClangTidy warning about moving from
        // `k` and then using `k` unsequenced. This is safe because the move is
        // into a forwarding reference and insert_unique guarantees that `key`
        // is never referenced after consuming `args`.
        const key_type& key_ref = k;
        return this->tree_.insert_unique(
            key_ref, std::piecewise_construct,
    ...
    iterator try_emplace(const_iterator hint, key_type&& k, Args&&... args) {
        // Note: `key_ref` exists to avoid a ClangTidy warning about moving from
        // `k` and then using `k` unsequenced. This is safe because the move is
        // into a forwarding reference and insert_hint_unique guarantees that
        // `key` is never referenced after consuming `args`.
        const key_type& key_ref = k;
        return this->tree_
            .insert_hint_unique(
...
public:
    ...
    iterator insert(node_type&& node) {
        if (!node)
            return this->end();
        iterator res = this->tree_.insert_multi(
            params_type::key(CommonAccess::GetSlot(node)),
            CommonAccess::GetSlot(node));
        CommonAccess::Destroy(&node);
        return res;
...
class btree_set :
    public priv::btree_set_container<priv::btree<priv::set_params<
...
class btree_multiset :
    public priv::btree_multiset_container<priv::btree<priv::set_params<
...
class btree_map :
    public priv::btree_map_container<priv::btree<priv::map_params<
...
class btree_multimap :
    public priv::btree_multimap_container<priv::btree<priv::map_params<
