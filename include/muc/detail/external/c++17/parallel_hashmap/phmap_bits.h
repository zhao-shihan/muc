#if !defined(muc_impl_phmap_bits_h_guard_)
#define muc_impl_phmap_bits_h_guard_
...
PHMAP_BASE_INTERNAL_FORCEINLINE uint32_t CountLeadingZeros64(uint64_t n) {
    ...
    //  ARM64: clz
    //  PPC: cntlzd
    static_assert(sizeof(unsigned long long) ==
                      sizeof(n), // NOLINT(runtime/int)
...
PHMAP_BASE_INTERNAL_FORCEINLINE uint32_t
CountTrailingZerosNonZero64Slow(uint64_t n) {
    uint32_t c = 63;
    n &= ~n + 1;
...
PHMAP_BASE_INTERNAL_FORCEINLINE uint32_t
CountTrailingZerosNonZero64(uint64_t n) {
...
PHMAP_BASE_INTERNAL_FORCEINLINE uint32_t
CountTrailingZerosNonZero32Slow(uint32_t n) {
    uint32_t c = 31;
    n &= ~n + 1;
    if (n & 0x0000FFFF)
        c -= 16;
...
PHMAP_BASE_INTERNAL_FORCEINLINE uint32_t
CountTrailingZerosNonZero32(uint32_t n) {
#if defined(_MSC_VER) && !defined(__clang__)
    unsigned long result = 0; // NOLINT(runtime/int)
